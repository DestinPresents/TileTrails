...
function levelTarget(level){if(level<=25)return 15;if(level<=50)return 18;if(level<=75)return 21;if(level<=100)return 24;return Math.min(180,24+Math.floor((level-100)/40)*6)}
...
function renderTray(){
  ...
  for(let i=0;i<7;i++){
    ...
    if(i===6&&!state.extraSlot){
      ...
      s.innerHTML='<span class="slot-plus">＋</span><small>30</small>';
      s.title="Buy extra slot for 30 coins";
...
function howToPlay(){modal(`<div class="settings-head"><div class="settings-icon">❓</div><div><div class="eyebrow">QUICK GUIDE</div><h2>HOW TO PLAY</h2></div></div><div class="howto-list"><div>1️⃣ Tap an uncovered tile.</div><div>2️⃣ Collect 3 identical tiles.</div><div>3️⃣ Three matching tiles disappear.</div><div>4️⃣ Keep the tray from filling.</div><div>5️⃣ Use Undo, Shuffle and Hint wisely.</div></div><button class="modal-action" onclick="closeModal()">GOT IT!</button>`)}
...
function buyExtraSlot(){
  ...
  if(state.coins<30){playSound('lose');vibrate([50]);showToast('NOT ENOUGH COINS','bad');return;}
  state.coins-=30;state.extraSlot=true;save();updateCoins();renderTray();playSound('buy');vibrate([16,25]);showToast('EXTRA SLOT UNLOCKED','buy');
}
